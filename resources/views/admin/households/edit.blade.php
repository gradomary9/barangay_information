<x-layout title="Edit Household">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <x-card title="Update Household">
                <form action="{{ route('households.update', $household) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="household_head_name" value="{{ old('household_head_name', $household->household_head_name ?? trim(($household->head?->first_name ?? '') . ' ' . ($household->head?->last_name ?? ''))) }}" class="form-control @error('household_head_name') is-invalid @enderror" placeholder="Type household head name">
                        @error('household_head_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Address</label>
                        <textarea name="address" class="form-control @error('address') is-invalid @enderror" rows="3" required>{{ old('address', $household->address) }}</textarea>
